// src/fix-engine.js — fix_security tool handler
import { z } from "zod";
...
export async function fixSecurity({ file_path, verbosity }) {
  ...
  const rawResult = await runAnalyzerAsync(file_path);

  if (rawResult.error) {
    return {
      content: [{
        type: "text",
        text: JSON.stringify({ message: "Error scanning file", details: rawResult })
      }]
    };
  }
  ...
  if (!Array.isArray(rawFindings) || rawFindings.length === 0) {
    return {
      content: [{
        type: "text",
        text: JSON.stringify({ message: "No security issues found", fixes_applied: 0, file: file_path })
      }]
    };
  }
  ...
  for (const issue of sortedIssues) {
    ...
    if (lineIndex >= 0 && lineIndex < lines.length) {
      ...
      if (fix && fix.fixed && fix.fixed !== originalLine) {
