...
describe('readFileAtRef and getChangedFiles against a real git repo', () => {
  ...
  beforeAll(() => { // nosec - test fixture setup, not template escaping.
    repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cr-agent-diff-test-')); // nosec - test fixture setup.
    git(['init', '--quiet']);
    git(['config', 'user.email', 'test@example.com']);
    git(['config', 'user.name', 'Test']);

    // Base commit: a file with original content.
    fs.mkdirSync(path.join(repoDir, 'pkg'));
    fs.writeFileSync(path.join(repoDir, 'app.py'), 'def greet():\n    return "hello"\n');
    fs.writeFileSync(path.join(repoDir, 'pkg', 'app.py'), 'def nested():\n    return "base"\n');
    git(['add', '.']);
    ...
    fs.writeFileSync(path.join(repoDir, 'pkg', 'app.py'), 'def nested(name):\n    return f"head {name}"\n');
    ...
    fs.writeFileSync(path.join(repoDir, 'pkg', 'app.py'), 'def unrelated_nested():\n    pass\n');
    fs.writeFileSync(path.join(repoDir, 'only-on-third-commit.py'), 'x = 1\n');
    git(['add', '.']);
    git(['commit', '--quiet', '-m', 'unrelated third commit']);

    (globalThis as unknown as { __testCommits: { base: string; head: string } }).__testCommits = { // nosec - test fixture state.
      base: baseCommit,
      head: headCommit,
    };
  });
  ...
  it('getChangedFiles finds the diff correctly regardless of what is currently checked out', () => {
    ...
    expect(files.map((f) => f.filePath)).toEqual(['app.py', 'pkg/app.py']);
  });
  ...
  it('readFileAtRef reads from the repository root when projectRoot is a subdirectory', () => {
    const { head } = (globalThis as unknown as { __testCommits: { base: string; head: string } }).__testCommits;
    const pkgDir = path.join(repoDir, 'pkg');

    const files = getChangedFiles(pkgDir, `${head}~1`, head);
    expect(files.map((f) => f.filePath)).toEqual(['app.py']);

    const headContent = readFileAtRef(pkgDir, head, 'app.py');
    expect(headContent).toContain('def nested(name):');
    expect(headContent).not.toContain('unrelated_nested');
  });
});
