...
export function getChangedFiles(projectRoot: string, base: string, head: string): FileDiff[] {
  let raw: string;
  try {
    raw = execFileSync( // nosec - fixed git args, no shell.
      'git',
...
export function parseDiff(diffText: string): FileDiff[] {
  ...
  for (const block of fileBlocks) {
    ...
    let hm: RegExpExecArray | null; // nosec - scanner internals, not template escaping.
    while ((hm = hunkRe.exec(block)) !== null) { // nosec - scanner internals, not template escaping.
...
export function readFileAtRef(projectRoot: string, ref: string, relativePath: string): string {
  // Git accepts forward slashes in <ref>:<path> on all platforms. When
  // projectRoot is a subdirectory of the repository, git show still resolves
  // paths from the repository root, so prepend the cwd's git prefix.
  const prefix = execFileSync(
    'git',
    ['rev-parse', '--show-prefix'],
    { cwd: projectRoot, encoding: 'utf-8' },
  ).trim();
  const normalizedRelativePath = relativePath.replace(/\\/g, '/').split(path.sep).join('/');
  const gitPath = `${prefix}${normalizedRelativePath}`.replace(/^\/+/, '');
  return execFileSync(
    'git',
    ['show', `${ref}:${gitPath}`],
    { cwd: projectRoot, encoding: 'utf-8', maxBuffer: 64 * 1024 * 1024 },
